...
- **`qaApproved`** — `mr.labels.includes('qa_approved')`. Used as one half of the `readyToMerge` gate, and rendered as a green badge (`.mr-badge-success`) — but only in the "Awaiting Review" section. Unlike every other badge (computed once, shown wherever true), this one needs a `showQaApprovedBadge` prop threaded `App.tsx` → `MergeRequestSection` → `MergeRequestRow`, because `qaApproved` is computed for *every* MR (same as `qaPending`), and showing "QA approved" on an MR you're just reviewing/authoring (unrelated to your own assignee queue) would be noise. If you add another section-scoped badge, follow this same prop-threading pattern rather than gating on category membership inside `MergeRequestRow` itself.
